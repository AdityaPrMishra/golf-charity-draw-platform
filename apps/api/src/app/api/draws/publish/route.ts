...
import { notifyDrawPublished } from "@/lib/draws/notify";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await runDraw({ draw_month, draw_type, status: "published" });
    if (!result.ok) {
    ...
    notifyDrawPublished({
      ...
      drawnNumbers: result.draw.drawn_numbers,
    }).catch((e) => {
      // eslint-disable-next-line no-console
      console.error("[notifyDrawPublished] failed", e);
    });

    return withCors(NextResponse.json(result), origin);
  } catch (e) {
    const message = e instanceof Error ? e.message : "Draw publish failed";
