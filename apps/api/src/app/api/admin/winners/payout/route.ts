...
import { payoutConfirmed } from "@/lib/email/templates";
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => null)) as
    | { entry_id?: string; status?: "paid" | "unpaid" }
    | null;
  ...
  if (profErr) {
    ...
    console.error("[admin/winners/payout] profile lookup failed", profErr);
  }
  ...
  if (email && status === "paid") {
    // eslint-disable-next-line no-console
    console.log("[email] payoutConfirmed", { to: email, entryId });
    await sendEmail({
      to: email,
      subject: `Payout confirmed (${drawMonth})`,
      html: payoutConfirmed(drawMonth, process.env.NEXT_PUBLIC_APP_URL ?? ""),
    });
  }

  return withCors(NextResponse.json({ ok: true, emailed: Boolean(email) && status === "paid" }), origin);
}
